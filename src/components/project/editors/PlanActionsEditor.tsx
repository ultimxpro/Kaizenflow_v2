...
// Interface locale pour les actions avec assignee_ids
interface Action {
    id: string;
    title: string;
    description?: string;
...
const AssigneeAvatars = ({ assignee_ids, users }: { assignee_ids: string[], users: User[] }) => {
    if (!assignee_ids || assignee_ids.length === 0) {
        return <div className="flex items-center text-xs text-gray-400">Non assigné</div>;
    }
...
const ActionCard = ({ action, users, onDragStart, onClick }: { action: Action, users: User[], onDragStart: (e: React.DragEvent, action: Action) => void, onClick: (action: Action) => void }) => {
    ...
    return (
        <div
            ...
            className={`bg-white/90 backdrop-blur-sm border border-gray-200/50 rounded-lg shadow-sm mb-1.5 border-l-4 ${config.color} p-2.5 hover:shadow-md hover:border-gray-300/70 cursor-pointer transition-all duration-200`}
        >
            <div className="flex justify-between items-start mb-1.5">
                <AssigneeAvatars assignee_ids={action.assignee_ids} users={users} />
                <span className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${config.a3Color}`}>
                    {config.icon}
                </span>
            </div>
            <h3 className="font-semibold text-gray-800 text-sm mb-1.5">{action.title}</h3>
...
const ActionModal = React.memo(({ isOpen, onClose, onSave, action, projectMembers }: { 
    ...
    useEffect(() => {
        ...
        const initialData = action ? {
            ...
            title: '', description: '', assignee_ids: [], status: 'À faire' as ActionStatus,
            type: 'simple' as ActionType, due_date: '', start_date: initialStartDate,
            effort: 5, gain: 5
        };
    ...
    const toggleAssignee = (userId: string) => {
        setFormData(prev => {
            ...
            let newLeaderId = (prev.leader_id && newAssignees.includes(prev.leader_id)) ? prev.leader_id : (newAssignees[0] || undefined);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 w-full max-w-4xl text-gray-800 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-gray-900">{action ? "Modifier l'action" : "Créer une action"}</h2>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={(e) => { e.preventDefault(); onSave(formData as Action); }} className="space-y-6">
                    <PDCASection title="Description" icon={<Layers size={18} className="text-blue-600" />}>
                        <div className="space-y-4">
                            <input
                                ...
                                className="p-3 w-full border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none"
                                required
                            />
                            <textarea
                                name="description"
                                ...
                                className="p-3 w-full border border-gray-300 rounded-lg h-24 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none resize-none"
                            />
                        </div>
                    </PDCASection>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="relative">
                                                ...
                                                />
                                                {isLeader && (
                                                    <div className="absolute -top-1 -right-1 w-5 h-5 bg-yellow-400 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm">
                                                        👑
                                                    </div>
                                                )}
                                            </div>
                                            <div>
                                                ...
                                                <div className="text-sm text-gray-500">
                                                    {isSelected ? (isLeader ? 'Leader' : 'Membre') : 'Non assigné'}
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            {isSelected && (
                                                <button
                                                    ...
                                                    onClick={(e) => {
                                                        ...
                                                        setFormData(prev => ({
                                                            ...prev,
                                                            leader_id: prev.leader_id === user.id ? undefined : user.id
                                                        }));
                                                    }}
                        ...
                        {(formData.assignee_ids || []).length === 0 && (
                            <div className="text-center py-8 text-gray-500">
                                ...
                                <p className="text-sm">Sélectionnez les membres de l'équipe</p>
                            </div>
                        )}
                    </div>

                    <PDCASection title="Détails" icon={<Table size={18} className="text-blue-600" />}>
                        <div className="space-y-6">
                            <div>
                                <label className="text-sm font-medium text-gray-700 flex items-center mb-3">
                                    <Activity size={14} className="mr-2 text-gray-500"/> Statut
                                </label>
                                <div className="flex gap-3">
                                    <button
                                        type="button"
                                        onClick={() => setFormData(p => ({...p, status: 'À faire'}))}
                                        className={`py-3 px-6 rounded-lg flex-1 font-medium transition-all ${
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setFormData(p => ({...p, status: 'Fait'}))}
                                        className={`py-3 px-6 rounded-lg flex-1 font-medium transition-all ${
                            ...
                            <div>
                                <label className="text-sm font-medium text-gray-700 flex items-center mb-3">
                                    <Tag size={14} className="mr-2 text-gray-500"/> Type d'action
                                </label>
                                <div className="grid grid-cols-3 gap-3">
                                    {Object.entries(actionTypeConfig).map(([key, config]) => (
                                        <button
                                            type="button"
                                            key={key}
                                            onClick={() => setFormData(p => ({...p, type: key as ActionType}))}
                                            className={`py-3 px-4 rounded-lg flex items-center justify-center gap-2 font-medium transition-all ${
                                                formData.type === key
                    ...
                    <PDCASection title="Planification" icon={<Calendar size={18} className="text-blue-600" />}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="text-sm font-medium text-gray-700 mb-3 block">Date de début</label>
                                <input
                                    type="date"
                                    name="start_date"
                            ...
                            <div>
                                <label className="text-sm font-medium text-gray-700 mb-3 block">Durée</label>
                                <div className="flex gap-3">
                                    <input
                                        type="number"
                                        min="1"
                                        ...
                                        className="p-3 w-20 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none"
                                    />
                                    <select
                                        value={durationUnit}
                                        onChange={(e) => setDurationUnit(e.target.value as 'days' | 'weeks' | 'months')}
                                        className="p-3 border border-gray-300 rounded-lg flex-1 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none"
                                    >
                                ...
                                {formData.due_date && formData.start_date && (
                                    <p className="text-xs text-blue-600 mt-3 bg-blue-50 p-3 rounded-lg">
                                        <span className="font-medium">Période :</span> {new Date(formData.start_date + 'T00:00:00').toLocaleDateString('fr-FR')} au {new Date(formData.due_date + 'T00:00:00').toLocaleDateString('fr-FR')}
                                    </p>
                                )}
                            </div>
                        </div>
                    ...
                    <PDCASection title="Priorisation" icon={<GanttChartSquare size={18} className="text-blue-600" />}>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
                            <div className="space-y-6">
                                <div>
                                    <div className="flex justify-between items-center mb-3">
                                        <label className="text-sm font-medium text-gray-700">Effort (Complexité)</label>
                                        <span className="text-sm font-semibold text-blue-600 bg-blue-50 px-2 py-1 rounded">{formData.effort || 5}/10</span>
                                    </div>
                                    <input
                                        type="range"
                                        name="effort"
                                        ...
                                        onChange={e => handleRangeChange('effort', e.target.value)}
                                        className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider-thumb focus:outline-none focus:ring-2 focus:ring-blue-200"
                                    />
                                </div>
                                <div>
                                    <div className="flex justify-between items-center mb-3">
                                        <label className="text-sm font-medium text-gray-700">Gain (Impact)</label>
                                        <span className="text-sm font-semibold text-green-600 bg-green-50 px-2 py-1 rounded">{formData.gain || 5}/10</span>
                                    </div>
                                    <input
                                        type="range"
                                        name="gain"
                                        min="1"
                                        max="10"
                                        value={formData.gain || 5}
                                        onChange={e => handleRangeChange('gain', e.target.value)}
                                        className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider-thumb focus:outline-none focus:ring-2 focus:ring-green-200"
                                    />
                                </div>
                            </div>
                            <div className="text-center">
                                <p className="text-sm font-medium text-gray-700 mb-3">Position dans la matrice</p>
                                <div className={`p-4 rounded-lg font-medium transition-colors ${currentQuadrant.color}`}>
...
const HomeView = ({ actions, onUpdateAction, users, onCardClick }: { actions: Action[], onUpdateAction: (actionId: string, updates: Partial<Action>) => void, users: User[], onCardClick: (action: Action) => void }) => {
    ...
    const handleDrop = (e: React.DragEvent, targetType: ActionType) => {
        ...
        target.classList.remove('bg-blue-50/50', 'border-blue-400', 'shadow-xl');
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full" onDragEnd={() => setDraggedItem(null)}>
            {Object.entries(columns).map(([type, items]) => {
                ...
                return (
                    <div key={type} className={`flex flex-col rounded-2xl transition-all duration-200 ${config.lightBg} h-full overflow-hidden shadow-lg border border-gray-200/30`}
                         onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, type as ActionType)}
                         onDragEnter={(e) => (e.currentTarget as HTMLDivElement).classList.add('bg-blue-50/50', 'border-blue-400', 'shadow-xl')}
                         onDragLeave={(e) => (e.currentTarget as HTMLDivElement).classList.remove('bg-blue-50/50', 'border-blue-400', 'shadow-xl')}>
                        <h2 className={`font-bold p-4 flex items-center gap-3 ${config.textColor} bg-white/80 backdrop-blur-sm`}>
                            <span className="text-xl">{config.icon}</span>
                            <span className="text-base">{config.name}</span>
                            <span className="text-sm font-normal text-gray-500 ml-auto bg-white/80 rounded-full px-2 py-1 shadow-sm">{items.length}</span>
                        </h2>
...
const KanbanByPersonView = ({ actions, onUpdateAction, users, onCardClick }: { actions: Action[], onUpdateAction: (actionId: string, updates: Partial<Action>) => void, users: User[], onCardClick: (action: Action) => void }) => {
    ...
    const handleDrop = (e: React.DragEvent, targetStatus: ActionStatus) => {
        ...
        e.stopPropagation();
        const target = e.currentTarget as HTMLDivElement;
        target.classList.remove('bg-blue-50', 'ring-2', 'ring-blue-400');
                ...
                {(['À faire', 'Fait'] as ActionStatus[]).map(status => {
                    ...
                    return (
                        <div
                            ...
                            onDragEnter={(e) => (e.currentTarget as HTMLDivElement).classList.add('bg-blue-50', 'ring-2', 'ring-blue-400')}
                            onDragLeave={(e) => (e.currentTarget as HTMLDivElement).classList.remove('bg-blue-50', 'ring-2', 'ring-blue-400')}
                        >
...
const MatrixView = ({ actions, onUpdateAction, users, onCardClick }: { actions: Action[], onUpdateAction: (actionId: string, updates: Partial<Action>) => void, users: User[], onCardClick: (action: Action) => void }) => {
    ...
    const handleDrop = (e: React.DragEvent, quadrant: string) => {
        ...
        e.stopPropagation();
        const target = e.currentTarget as HTMLDivElement;
        target.classList.remove('ring-4', 'ring-green-400');
                        ...
                        return (
                            <div key={action.id} className={`flex border-b border-gray-100 ${rowIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors`}>
                                ...
                                <div className="relative flex items-center" style={{ width: `${totalWidth}px`, height: '60px' }}>
                                    ...
                                    <div
                                       className={`absolute top-1/2 transform -translate-y-1/2 h-6 ${config.barBg} rounded cursor-pointer flex items-center shadow-sm hover:shadow-md transition-shadow`}
...
export const PlanActionsEditor: React.FC<PlanActionsEditorProps> = ({ module, onClose }) => {
    ...
    const currentProjectMembers = useMemo(() => {
        const memberIds = projectMembers
            .filter(pm => pm.project_id === module.project_id)
            .map(pm => pm.user_id);
        return allUsersInApp
            .filter(user => memberIds.includes(user.id))
            .map(user => ({
                id: user.id,
                nom: user.nom,
                avatarUrl: user.signedAvatarUrl || user.avatar_url
            }));
    }, [projectMembers, allUsersInApp, module.project_id]);

// Transformer les données Supabase en format du composant
const actions = useMemo(() => {
    return allActions
        .filter(action => action.project_id === module.project_id)
        .map(action => {
            const assignees = actionAssignees.filter(aa => aa.action_id === action.id);
            const leader = assignees.find(aa => aa.is_leader);
            
            return {
                ...action,
                assignee_ids: assignees.map(aa => aa.user_id),
                leader_id: leader?.user_id
            };
        });
}, [allActions, actionAssignees, module.project_id]);
...
useEffect(() => {
    ...
    try {
        if (actionData.id && actions.some(a => a.id === actionData.id)) {
            ...
            // Gérer les assignations pour l'action modifiée
            const currentAssignees = actionAssignees.filter(aa => aa.action_id === actionData.id);
            const newAssigneeIds = actionData.assignee_ids || [];
            
            // Supprimer les anciennes assignations qui ne sont plus présentes
            for (const assignee of currentAssignees) {
                if (!newAssigneeIds.includes(assignee.user_id)) {
                    await removeActionAssignee(actionData.id, assignee.user_id);
                }
            }
            
            // Ajouter/mettre à jour les nouvelles assignations
            for (const userId of newAssigneeIds) {
                const existingAssignee = currentAssignees.find(aa => aa.user_id === userId);
                const isLeader = userId === actionData.leader_id;
                
                if (!existingAssignee) {
                    // Nouvelle assignation
                    await addActionAssignee(actionData.id, userId, isLeader);
                } else if (existingAssignee.is_leader !== isLeader) {
                    // Mettre à jour le statut de leader
                    await removeActionAssignee(actionData.id, userId);
                    await addActionAssignee(actionData.id, userId, isLeader);
                }
            }
        } else {
            ...
            // Ajouter les assignations pour la nouvelle action
...
}, [actions, actionAssignees, createAction, updateAction, addActionAssignee, removeActionAssignee, module.project_id]);
                    ...
                    <div className="p-6 border-b border-gray-200/50 flex-shrink-0">
                        <div className="flex flex-wrap justify-between items-center gap-4">
                            ...
                            <button
                                ...
                                className="flex items-center space-x-3 px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl hover:from-green-600 hover:to-emerald-700 transition-all duration-200 shadow-lg hover:shadow-xl"
                            >
